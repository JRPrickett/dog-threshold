...
const listing = await fetch(`${base}?name=${encodeURIComponent(name)}`, { headers });
const list = await listing.json();
if (!listing.ok || !list.success) throw new Error("Could not list account databases; check D1 permissions.");
let database = list.result.find(db => db.name === name);
if (!database) {
  const response = await fetch(base, { method: "POST", headers, body: JSON.stringify({ name }) });
  const result = await response.json();
  if (!response.ok || !result.success) throw new Error("Could not create account database; check D1 permissions.");
