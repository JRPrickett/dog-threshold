  ...
  return { get requests() { return requests; },
    async attach(context: BrowserContext) {
      ...
      await context.route("**/api/**", async route => {
        ...
        if (!state.online) { await route.abort(); return; }
        const respond = (json: unknown, status = 200) => route.fulfill({ status, json });
        if (path === "/api/account/status") return respond({ available: true });
        if (path === "/api/auth/get-session") return respond(state.signedIn ? { user: { id: state.account, email: `${state.account}@example.test` } } : null);
        if (path.endsWith("send-verification-otp")) return respond({ success: true });
        if (path.endsWith("sign-in/email-otp")) { state.signedIn = true; return respond({ success: true }); }
        if (path.endsWith("sign-out")) { state.signedIn = false; return respond({ success: true }); }
        if (path === "/api/sync") {
          requests++;
          if (!state.signedIn) return respond({ error: "Sign in again" }, 401);
          const body = route.request().postDataJSON() as { cursor: number; operations: SyncOperation[] };
          ...
          for (const operation of body.operations) {
            ...
            else if ((existing?.revision ?? 0) !== operation.base) conflicts.push({ id: operation.id, record: existing! });
            else {
              const record = { key: operation.key, revision: changes.length + 1, value: operation.value };
              records.set(record.key, record); changes.push(record); receipts.set(operation.id, record.revision);
              accepted.push({ id: operation.id, revision: record.revision });
            }
          }
          return respond({ accepted, conflicts, changes: changes.filter(item => item.revision > body.cursor), cursor: changes.length, hasMore: false });
        }
...
test("account import is explicit, controls fit mobile, and sign-out prevents account mixing", async ({ page, context }, info) => {
  const server = cloud(); const state = await server.attach(context);
  await setup(page);
  await page.getByRole("button", { name: "More", exact: true }).click();
  await page.screenshot({ path: info.outputPath("account-signin.png"), fullPage: true });
  const size = await page.getByLabel("Email address").evaluate(node => parseFloat(getComputedStyle(node).fontSize));
  expect(size).toBeGreaterThanOrEqual(16);
  await signIn(page);
  await expect(page.getByText("Connect this training log?", { exact: true })).toBeVisible();
  expect(server.requests).toBe(0);
  await page.getByRole("button", { name: "Connect and upload this log" }).click();
  await expect(page.getByText(/Last synced/)).toBeVisible();
  await page.screenshot({ path: info.outputPath("account-connected.png"), fullPage: true });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.getByRole("button", { name: "Sign out", exact: true }).click();
  state.account = "different";
  await signIn(page);
  await expect(page.getByText(/log belongs to another account/)).toBeVisible();
  await expect(page.getByRole("button", { name: "Connect and upload this log" })).toHaveCount(0);
});
test("offline local save syncs later and restores on a second device without duplicated sessions", async ({ page, context, browser }, info) => {
  const server = cloud(); const state = await server.attach(context);
  await setup(page);
  await page.getByRole("button", { name: "More", exact: true }).click(); await signIn(page);
  await page.getByRole("button", { name: "Connect and upload this log" }).click();
  await expect(page.getByText(/Last synced/)).toBeVisible();
  state.online = false;
  ...
  const second = await browser.newContext({ ...info.project.use });
  try {
    await server.attach(second);
    const other = await second.newPage(); await other.goto("/app/");
    await other.getByRole("button", { name: "Already have an account? Sign in" }).click(); await signIn(other);
    await other.getByRole("button", { name: "Restore from my account" }).click();
