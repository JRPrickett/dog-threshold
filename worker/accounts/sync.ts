import {
  ...
  type RemoteRecord,
  type SyncReply,
} from "../../app-v2/src/account/protocol";
interface Row {
  record_key: string;
  revision: number;
  value_json: string | null;
}
const record = (row: Row): RemoteRecord => ({
  key: row.record_key,
...
export async function sync(
  ...
  for (const op of body.operations) {
    if (op.value && keyFor(op.value) !== op.key)
      throw new Error("Invalid record key");
    if (!/^(profile|scenario|session|cue):/.test(op.key))
      throw new Error("Invalid record key");
  }
  for (const op of body.operations) {
    ...
    await db.batch([
      db
        .prepare(
          `INSERT OR IGNORE INTO sync_changes(user_id,mutation_id,record_key,value_json)
      ...
      db
        .prepare(
          `INSERT INTO sync_records(user_id,record_key,revision,value_json)
    ...
    const accepted = await db
      .prepare("SELECT seq FROM sync_changes WHERE user_id=? AND mutation_id=?")
      .bind(userId, op.id)
      .first<{ seq: number }>();
    if (accepted) reply.accepted.push({ id: op.id, revision: accepted.seq });
      ...
      const row = await db
        .prepare(
          "SELECT record_key,revision,value_json FROM sync_records WHERE user_id=? AND record_key=?",
        )
        .bind(userId, op.key)
        .first<Row>();
      reply.conflicts.push({
        id: op.id,
        record: row ? record(row) : { key: op.key, revision: 0, value: null },
      });
    }
  }
  const { results } = await db
    .prepare(
      "SELECT record_key,seq AS revision,value_json FROM sync_changes WHERE user_id=? AND seq>? ORDER BY seq LIMIT 201",
    )
    .bind(userId, body.cursor)
    .all<Row>();
