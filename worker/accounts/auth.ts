...
export function accountsConfigured(env: AccountEnv): boolean {
  return env.ACCOUNTS_ENABLED === "true" && Boolean(env.ACCOUNTS_DB &&
    env.AUTH_ORIGIN?.startsWith("https://") && (env.BETTER_AUTH_SECRET?.length ?? 0) >= 32 &&
    env.RESEND_API_KEY && env.AUTH_EMAIL_FROM);
}

export function createAuth(env: AccountEnv, deliver = async (email: string, otp: string) => {
  ...
  return betterAuth({
    appName: "SettledSolo", database: env.ACCOUNTS_DB!,
    baseURL: env.AUTH_ORIGIN!, basePath: "/api/auth", secret: env.BETTER_AUTH_SECRET!,
    trustedOrigins: [env.AUTH_ORIGIN!],
    emailAndPassword: { enabled: false },
    session: { expiresIn: 60 * 60 * 24 * 30, updateAge: 60 * 60 * 24, freshAge: 600 },
    user: { deleteUser: { enabled: true, beforeDelete: async (user) => {
      await env.ACCOUNTS_DB!.prepare("DELETE FROM verification WHERE identifier=?").bind(`sign-in-otp-${user.email}`).run();
    } } },
    advanced: { useSecureCookies: true, cookiePrefix: "settledsolo", ipAddress: { ipAddressHeaders: ["cf-connecting-ip"] } },
    ...
    plugins: [emailOTP({ otpLength: 6, expiresIn: 300, allowedAttempts: 3, storeOTP: "hashed",
      async sendVerificationOTP({ email, otp, type }) {
        if (type !== "sign-in") throw new Error("Unsupported verification type");
